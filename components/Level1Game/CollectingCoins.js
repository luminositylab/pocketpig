...
const coin1 = Matter.Bodies.circle(wp("70%"), hp("50%"), 40, {
  ...coinSettings,
  label: "coin1",
});
const coin2 = Matter.Bodies.circle(wp("28%"), hp("40%"), BALL_SIZE, {
  ...coinSettings,
  label: "coin2",
});
const coin3 = Matter.Bodies.circle(wp("30%"), hp("10%"), BALL_SIZE, {
  ...coinSettings,
  label: "coin3",
});
const coin4 = Matter.Bodies.circle(wp("75%"), hp("20%"), BALL_SIZE, {
  ...coinSettings,
  label: "coin4",
});
const coin5 = Matter.Bodies.circle(wp("30%"), hp("50%"), 20, {
  ...coinSettings,
  label: "coin5",
});
const coin6 = Matter.Bodies.circle(wp("60%"), hp("30%"), 20, {
  ...coinSettings,
  label: "coin6",
});
...
const topWall = Matter.Bodies.rectangle(wp("30%"), hp("20%"), 100, 1, {
  ...wallSettings,
  label: "topWall",
});
const bottomWall = Matter.Bodies.rectangle(wp("65%"), hp("30%"), 100, 1, {
  ...wallSettings,
  label: "bottomWall",
});
...
const rightWall = Matter.Bodies.rectangle(wp("72%"), hp("60%"), 100, 1, {
  ...wallSettings,
  label: "rightWall",
});
...
export default class CollectingCoins extends PureComponent {
  ...
  componentDidMount() {
    ...
    setTimeout(() => {
      Matter.Sleeping.set(coin1, false);
    }, 1000);
    setTimeout(() => {
      Matter.Sleeping.set(coin2, false);
    }, 2000);
    setTimeout(() => {
      Matter.Sleeping.set(coin3, false);
    }, 3000);
    setTimeout(() => {
      Matter.Sleeping.set(coin4, false);
    }, 4000);
    setTimeout(() => {
      Matter.Sleeping.set(coin5, false);
    }, 4000);
    setTimeout(() => {
      Matter.Sleeping.set(coin6, false);
    }, 4000);
  ...
  render() {
    return (
      <>
        <GameMap
          style={{ left: wp("7.25%"), bottom: hp("25%"), position: "absolute" }}
        />
        <GameEngine
        ...
        <View style={{ alignItems: "center" }}>
          <Text
            style={{ marginBottom: hp("0%"), fontWeight: "800", fontSize: 20 }}
          >
            ${this.state.myScore.toFixed(2)}
          </Text>
          <Text
            style={[
              { marginBottom: hp("0%"), fontWeight: "100", fontSize: 15 },
              this.state.firstHistory >= 0
                ? { color: "#3C885E" }
                : { color: "#FC8476" },
            ]}
          >
            {this.state.firstHistory == 0
              ? null
              : "$" + this.state.firstHistory.toFixed(2)}
          </Text>
          <Text
            style={[
              { marginBottom: hp("0%"), fontWeight: "100", fontSize: 15 },
              this.state.secondHistory >= 0
                ? { color: "#3C885E" }
                : { color: "#FC8476" },
            ]}
          >
            {this.state.secondHistory == 0
              ? null
              : "$" + this.state.secondHistory.toFixed(2)}
          </Text>
          <Text
            style={[
              { marginBottom: hp("7%"), fontWeight: "100", fontSize: 15 },
              this.state.thirdHistory >= 0
                ? { color: "#3C885E" }
                : { color: "#FC8476" },
            ]}
          >
            {this.state.thirdHistory == 0
              ? null
              : "$" + this.state.thirdHistory.toFixed(2)}
          </Text>
          <TouchableOpacity
            onPress={() => {
              if (this.state.myScore > 0.5) {
                this.props.navigation.navigate({
                  ...
                  params: {
                    ...
                    levelInformation:
                      this.props.route.params["levelInformation"],
                    coins: this.state.myScore,
                    landOwned: this.props.route.params["landOwned"],
                  },
                });
              } else {
                this.props.navigation.navigate({
